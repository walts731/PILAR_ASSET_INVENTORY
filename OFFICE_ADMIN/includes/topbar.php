...
// Fetch logged-in user info including office
...
$low_stock_stmt = $conn->prepare("
    SELECT id, asset_name AS product_name, quantity AS stock 
    FROM assets 
    WHERE type = 'consumable' AND quantity <= ? 
...
<div class="topbar d-flex flex-wrap justify-content-between align-items-center p-2 gap-2">
  ...
  <div class="order-3 order-sm-2 flex-grow-1">
    ...
    <!-- Display Office -->
    <?php if ($office_name): ?>
      <small class="d-block text-muted text-center text-sm-start"><?= htmlspecialchars($office_name) ?></small>
    <?php endif; ?>
  </div>
          ...
          <?php foreach ($low_stock_items as $item): ?>
            <li>
              <a class="dropdown-item small text-danger d-flex justify-content-between align-items-center"
                href="admin_dashboard.php?id=<?= $item['id'] ?>">
