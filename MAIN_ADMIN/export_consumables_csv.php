...
$filename = 'consumables_export' . $filter_suffix . '_' . date('Ymd_His') . '.csv';
...
// CSV column headers (match table columns in consumables tab)
fputcsv($out, [
  'Property No',
  'Description',
  'Category',
...
if ($res) {
  while ($r = $res->fetch_assoc()) {
    ...
    $stock_level = ($r['quantity'] <= $threshold) ? 'Low Stock' : 'Normal';
...
try {
  ...
  error_log("Failed to insert consumables CSV export into generated_reports: " . $e->getMessage());
}
