...
$stmtInsAsset = $conn->prepare("INSERT INTO assets 
    (asset_name, description, quantity, unit, status, acquisition_date, office_id, employee_id, red_tagged, last_updated, value, qr_code, type, image, serial_no, code, property_no, model, brand, inventory_tag, asset_new_id, category, end_user)
    VALUES (?, ?, 1, ?, ?, ?, ?, ?, ?, NOW(), ?, '', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
