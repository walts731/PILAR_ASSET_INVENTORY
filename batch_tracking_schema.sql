...
-- Drop existing foreign key constraints before recreating them
-- Note: This block is safe to run even if the constraints don't exist.
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;

ALTER TABLE `batches` 
    DROP FOREIGN KEY IF EXISTS `fk_batch_asset`,
    DROP FOREIGN KEY IF EXISTS `fk_batch_category`,
    DROP FOREIGN KEY IF EXISTS `fk_batch_creator`;

ALTER TABLE `batch_items`
    DROP FOREIGN KEY IF EXISTS `fk_batch_item_batch`,
    DROP FOREIGN KEY IF EXISTS `fk_batch_item_office`,
    DROP FOREIGN KEY IF EXISTS `fk_batch_item_employee`;

ALTER TABLE `batch_transactions`
    DROP FOREIGN KEY IF EXISTS `fk_transaction_batch`,
    DROP FOREIGN KEY IF EXISTS `fk_transaction_item`,
    DROP FOREIGN KEY IF EXISTS `fk_transaction_from_office`,
    DROP FOREIGN KEY IF EXISTS `fk_transaction_to_office`,
    DROP FOREIGN KEY IF EXISTS `fk_transaction_user`,
    DROP FOREIGN KEY IF EXISTS `fk_transaction_recipient`;

ALTER TABLE `batch_receipts`
    DROP FOREIGN KEY IF EXISTS `fk_receipt_batch`,
    DROP FOREIGN KEY IF EXISTS `fk_receipt_receiver`,
    DROP FOREIGN KEY IF EXISTS `fk_receipt_checker`;

ALTER TABLE `borrow_requests`
    DROP FOREIGN KEY IF EXISTS `fk_borrow_batch`,
    DROP FOREIGN KEY IF EXISTS `fk_borrow_batch_item`;
